                                        ...
                                        <div class="pd-img-wrap position-relative h-100">
                                            <div class="swiper-container quickviewSlider2 border rounded"
                                                ...
                                                @else
                                                    ...
                                                    @php
                                                       $wishCount =  App\Model\Wishlist::where('customer_id',auth('customer')->user()->id)->get()->where('product_id',$product['id'])->count();
                                                    @endphp
                                                      @if($wishCount > 0)
                                                      <!-- Button trigger modal -->
                                                        <a class="btn-wishlist add_to_wishlist wishlist_icon_active cursor-pointer wishlist-{{$product['id']}} {{($wishlist_status == 1?'wishlist_icon_active':'')}}" data-bs-toggle="modal" data-bs-target="#exampleModal" id="wishlist-{{$product['id']}}">
                                                            <i class="bi bi-heart"></i>
                                                        </a>
                                                      @else
                                                      <!-- Button trigger modal -->
                                                        <a class="btn-wishlist add_to_wishlist cursor-pointer wishlist-{{$product['id']}} {{($wishlist_status == 1?'wishlist_icon_active':'')}}" data-bs-toggle="modal" data-bs-target="#exampleModal" id="wishlist-{{$product['id']}}">
                                                            <i class="bi bi-heart"></i>
                                                        </a>
                                                      @endif
                                                                ...
                                                                <div class="modal-body">
                                                                    @if(isset($wishlistcategory) && $wishlistcategory->count() > 0)
                                                                        <div class="row mb-3">
                                                                            ...
                                                                            @php
                                                                             $wishlistPro = App\Model\Wishlist::where('customer_id',auth('customer')->id())->where('wishlist_category_id',$category->id)->where('product_id',$product->id)->first();
                                                                             $wishlistCount = App\Model\Wishlist::where('customer_id',auth('customer')->id())->where('product_id',$product['id'])->get()->count();
                                                                            @endphp
                                    ...
                                    <div class="col-lg-7">
                                        ...
                                        <div class="product-details-content position-relative">
                                            ...
                                            <div class="d-flex flex-wrap align-items-center gap-2 mb-3">
                                                <h2 class="product_title text-truncate">{{$product->name}}</h2>
                                                
                                                 <h5 class="product_title text-truncate">{{$product->disc}}</h5>
