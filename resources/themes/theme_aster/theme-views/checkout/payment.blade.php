                                ...
                                <ul class="option-select-btn flex-wrap gap-3">
                                    ...
                                    @endif
                                        ...
                                        @php 
                                            ...
                                            $gatewayId = "019856927";
                                            $secert = "pwxUSEo0DXoRfvsH";
                                            $hashable_string = "gatewayId=".$gatewayId.",amount=".$amount.",referenceId=".$order_id;
                                            $signature = base64_encode(hash_hmac('sha256',$hashable_string,$secert,true));
                                        @endphp
                                        <li>
                                            <form method="POST" action="{{ url('https://paymentapi.qib.com.qa/api/gateway/v2.0') }}">
                                                <input hidden name="action" value="capture" />
                                                <input hidden name="gatewayId" value="{{$gatewayId}}" />
                                                <input hidden name="signatureFields" value="gatewayId,amount,referenceId" />

                                                <input hidden name="signature" value="{{$signature}}" />
                                                <input hidden name="referenceId" value="{{$order_id}}" />
                                                <input hidden name="amount" value="{{$amount}}" />
                                                <input hidden name="currency" value="QAR" />
                                                <input hidden name="mode" value="LIVE" />
                                                <input hidden name="description" value="PRODUCT_DESC" />
                                                <input hidden name="returnUrl" value="{{url('/qib/callback')}}" />
                                                <input hidden name="name" value="{{$name}}" />
                                                <input hidden name="address" value="{{$user->street_address ?? 'no address found'}}" />
                                                <input hidden name="city" value="{{$user->city ?? 'no city found'}}" />
                                                <input hidden name="state" value="{{$user->city ?? 'no state found'}}" />
                                                <input hidden name="country" value="QA" />
                                                ...
                                                <label>
                                                    <button type="submit" class="payment-method border-0 d-flex align-iems-center gap-3">
                                                        <img width="100" src="{{ theme_asset('assets/img/payment/qib.png') }}" class="dark-support" alt="">
                                                    </button>
                                                </label>
                                            </form>
                                        </li>
