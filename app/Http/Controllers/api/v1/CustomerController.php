...
class CustomerController extends Controller
{
    ...
    public function add_category_to_wishlist(Request $request)
    {
         // return "yes";
        ...
        {
              return response()->json(['errors' => 'Name Input Is Required And Unique'], 403);
        }
    }
    
    public function categories_wishlist()
    {
        //  $wishlist_Category = WishlistCategory::where('user_id',auth()->id())->with('wishlists')->get();
         $wishlist_Category = WishlistCategory::where('user_id',auth()->id())->with(['wishlists' => function($q)
         {
             $q->select('product_id','wishlist_category_id');
         }])->get();
          return response()->json(
               [
                    
                    'data'      => $wishlist_Category
               ], 200);
    }
    
    
    public function update_category_to_wishlist(Request $request)
    {
        ...
        if($data && $data->count() > 0)
        {
         ...
         return response()->json(
               [
                    'message'   => translate('Your Wish List Folder Updated Successfully!'),
                    'data'      => $data
               ], 200);
        
    }
    
    public function remove_from_category_wishlist(Request $request)
    {
        // return $request;
    ...
    if($data && $data->count() > 0)
    {
        ...
        }else
        {
         ...
         DB::rollback();
           ...
           return response()->json(
               [
                    'message'   => translate('Your Wish List Folder Deleted Successfully!'),
                    'data'      => $data
               ], 200);
    }else
    {
    ...
    public function add_to_wishlist(Request $request)
    {
        ...
        $wishlist = Wishlist::where('customer_id', auth()->user()->id)->where('product_id', $request->product_id)->where('wishlist_category_id',$request->category_id)->first();
        $category = WishlistCategory::where('user_id', $request->user()->id)->where('id',$request->category_id)->first();
        ...
        if($category && $category->count() > 0)
        {
            if (empty($wishlist)) 
            {
                ...
                $wishlist->wishlist_category_id = $request->category_id;
                $wishlist->save();
                return response()->json(['message' => translate('successfully added!'),'category_id' => $request->category_id], 200);
            }else
            {
                 return response()->json(['message' => translate('Already in your wishlist'),'category_id' => $request->category_id], 409);
            }
    
        }else
        {
             return response()->json(['message' => translate('This Category Dosn,t Exsist!'),'category_id' => $request->category_id], 404);
        }
    }
