...
class WebController extends Controller
{
    ...
    //   return $request->id;
        ...
        ->where('customer_id', auth('customer')->id())->where('wishlist_category_id',$request->id)->paginate(15);
    ...
    public function storeWishlist(Request $request)
    {
        ...
        if ($request->ajax()) 
        {
            ...
            {
                $wishlist = Wishlist::where('customer_id', auth('customer')->id())->where('product_id', $request->product_id)->where('wishlist_category_id',$request->folder_id)->first();
                $wishlistProd = Wishlist::where(['product_id'=>$request->product_id, 'customer_id'=>auth('customer')->id()])->count();
                // return $wishlist;
                if ($wishlist) 
                {
                    $wishlist->delete();
                ...
                } else 
                {
                    ...
                    $wishlist->wishlist_category_id = $request->folder_id;
                    $wishlist->save();
