...
class ProductController extends BaseController
{
    ...
    {
        $minimum_tax = BusinessSetting::where('type', 'minimum_tax')->first()->value;
        $validator = Validator::make($request->all(), [
            ...
            'image'                => 'required',
            'tax'                   =>  'required|numeric|min:' . $minimum_tax,
            'tax_model'            => 'required',
            ...
            'disc.required'                     => 'Product main description is required!',
        ...
        $p->disc     = $request->disc[array_search('en', $request->lang)];
        $p->code     = $request->code;
        ...
        if($request->discount)
        {
              if($request->discount_type == 'percent')
              {
                  $p->price_after_dic    =  $request->unit_price - ($request->unit_price * $request->discount) / 100; 
              }else
              {
                 $p->price_after_dic =  $request->unit_price - $request->discount;
              }
        }
        ...
        if ($request->ajax()) {
            ...
            foreach ($request->lang as $index => $key) {
                ...
                if ($request->disc[$index] && $key != 'en') {
                    array_push($data, array(
                        'translationable_type' => 'App\Model\Product',
                        'translationable_id' => $p->id,
                        'locale' => $key,
                        'key' => 'disc',
                        'value' => $request->disc[$index],
                    ));
                }
            }
    ...
    public function update(Request $request, $id)
    {
        ...
        $min=\App\Model\BusinessSetting::where('type','min_commission')->first();
        
        if ($request['commission'] < $min->value) {
            $validator->after(function ($validator) {
                $validator->errors()->add('commission', 'the commission must be more or equal min commission!');
            });
        }
        ...
        $product->disc = $request->disc[array_search('en', $request->lang)];
        ...
        $product->commission           = $request->commission ?? null;
       ...
       if($request->discount)
        {
          if($request->discount_type == 'percent')
          {
              $product->price_after_dic    =  $request->unit_price - ($request->unit_price * $request->discount) / 100; 
          }else
          {
             $product->price_after_dic =  $request->unit_price - $request->discount;
          }
        }
