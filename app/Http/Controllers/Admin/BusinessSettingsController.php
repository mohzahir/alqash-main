...
class BusinessSettingsController extends Controller
{
    ...
    public function updateInfo(Request $request)
    {
        ...
        DB::table('business_settings')->updateOrInsert(['type' => 'minimum_tax'], [
            'value' => $request['minimum_tax']
        ]);
    ...
    public function sales_commission(Request $data)
    {
     ...
     public function min_commission(Request $data)
    {
        $validatedData = $data->validate([
            'min_commission' => 'required|min:0',
        ]);
        $sales_commission = BusinessSetting::where('type', 'min_commission')->first();

        if (isset($sales_commission)) {
            BusinessSetting::where('type', 'min_commission')->update(['value' => $data->min_commission]);
        } else {
            DB::table('business_settings')->insert(['type' => 'min_commission', 'value' => $data->min_commission]);
        }

        Toastr::success('min commission Updated successfully!');
        return redirect()->back();
    }
