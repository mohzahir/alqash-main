...
class Helpers
{
    ...
    public static function sales_commission($order)
    {
        ...
        $commission_amount = self::seller_sales_commission($order['seller_is'], $order['seller_id'], $order_total,$order);
    ...
    public static function sales_commission_before_order($cart_group_id, $coupon_discount)
    {
        ...
        $commission_amount = self::seller_sales_commission($carts[0]['seller_is'], $carts[0]['seller_id'], $cart_summery['order_total'],$carts);

        return $commission_amount;
    }

    public static function seller_sales_commission($seller_is, $seller_id, $order_total,$carts)
    {
        $commission_amount = 0;
        if ($seller_is == 'seller') {
            foreach($carts as $cart){
                if (isset($cart) && $cart['commission'] !== null) {
                    $commission = number_format($cart['commission'] * $cart['quantity'], 2);
                } else {
                    $commission_amount_m = Helpers::get_business_settings('sales_commission');
                    $commission = number_format(($cart->price / 100) * $commission_amount_m, 2) * $cart['quantity'];
                }
